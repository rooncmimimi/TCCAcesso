...
describe("ModeracaoService", () => {
  ...
  describe("listarBloqueados", () => {
    it("chama GET /usuarios/bloqueados com os parâmetros e devolve o envelope intacto", async () => {
      ...
      expect(clienteApi.get).toHaveBeenCalledWith("/usuarios/bloqueados", { params: { page: 1, limit: 20 } });
  ...
  describe("bloquear", () => {
    it("chama POST /usuarios/:id/bloquear e devolve o boolean do servidor", async () => {
      (clienteApi.post as jest.Mock).mockResolvedValue({ data: { sucesso: true, bloqueado: true } });

      await expect(ModeracaoService.bloquear("u2")).resolves.toBe(true);
      expect(clienteApi.post).toHaveBeenCalledWith("/usuarios/u2/bloquear");
    });
  ...
  describe("desbloquear", () => {
    it("chama DELETE /usuarios/:id/bloquear e devolve false", async () => {
      (clienteApi.delete as jest.Mock).mockResolvedValue({ data: { sucesso: true, bloqueado: false } });

      await expect(ModeracaoService.desbloquear("u2")).resolves.toBe(false);
      expect(clienteApi.delete).toHaveBeenCalledWith("/usuarios/u2/bloquear");
    });
  });
  ...
  describe("denunciar", () => {
    it("chama POST /denuncias com os dados e devolve só a denúncia", async () => {
      ...
      (clienteApi.post as jest.Mock).mockResolvedValue({ data: { sucesso: true, denuncia } });
      ...
      expect(clienteApi.post).toHaveBeenCalledWith("/denuncias", dados);
