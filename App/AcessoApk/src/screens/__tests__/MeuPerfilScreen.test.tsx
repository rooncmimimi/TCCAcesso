...
const mockAtualizarUsuarioAutenticado = jest.fn();
// Por padrão o usuário não tem `tipoUsuario`, o que cai no ramo de candidato; só o `describe` de
// roteamento, no fim do arquivo, muda isso.
const mockUseAuth = jest.fn().mockReturnValue({ atualizarUsuario: mockAtualizarUsuarioAutenticado });

jest.mock("../../autenticacao", () => ({
  useAutenticacao: () => mockUseAuth(),
}));

// O ramo de empresa (`PerfilEmpresaScreen`) usa `useNavigation()`, e os dois ramos vivem no mesmo
// componente, então o `NavigationContainer` real envolve a tela em todos os testes.
jest.mock("@react-navigation/native", () => ({
  ...jest.requireActual("@react-navigation/native"),
  useNavigation: () => ({ navigate: jest.fn() }),
}));

// Nos testes de candidato a tela de empresa nunca deve ser usada; o mock evita depender do
// `EmpresaService` real.
jest.mock("../../empresas", () => ({
...
describe("MeuPerfilScreen", () => {
  ...
  describe("dados pessoais", () => {
    it("editar, salvar chama atualizarUsuario e atualizarDadosPessoais, e volta pra visualização com o nome novo", async () => {
      ...
      // Sem isto, o resto do app (feed, menu do Perfil) continuaria com o nome antigo.
      expect(mockAtualizarUsuarioAutenticado).toHaveBeenCalled();
    });
    ...
    // O backend não normaliza a UF em `PUT /candidatos/:id`; sem a conversão, "sp" seria gravado em
  ...
  describe("experiências", () => {
    ...
    it("marcar 'emprego atual' esconde o campo de data de término", async () => {
      ...
      // `LinhaInterruptor` usa `Switch`: o evento é `valueChange`, não `press` (sem `onPress`).
      await act(async () => {
...
describe("MeuPerfilScreen — roteamento por tipoUsuario", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockUseAuth.mockReturnValue({ atualizarUsuario: mockAtualizarUsuarioAutenticado });
  });
  ...
  it("conta empresa: renderiza o perfil de empresa, NUNCA chama PerfilService.meuCandidato", async () => {
    ...
    mockUseAuth.mockReturnValue({
      atualizarUsuario: mockAtualizarUsuarioAutenticado,
      usuario: { id: "u9", nome: "ACME", email: "acme@exemplo.com", tipoUsuario: "empresa" },
    });
  ...
  it("conta candidato (ou sem usuario ainda resolvido): renderiza o perfil de candidato normalmente", async () => {
    mockCargaPadrao();
    const { EmpresaService } = jest.requireMock("../../empresas") as {
      EmpresaService: { meuPerfil: jest.Mock };
    };
