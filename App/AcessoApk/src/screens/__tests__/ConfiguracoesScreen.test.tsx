...
// "Usuários bloqueados" usa `useNavigation()`: o restante do módulo continua real e só
// `useNavigation` vira um mock controlável.
...
// `SecaoPrivacidade` tem um botão que navega para "Usuários bloqueados", então a tela precisa de um
// `NavigationContainer` real mesmo nos testes que não navegam.
...
describe("ConfiguracoesScreen", () => {
  beforeEach(async () => {
    ...
    mockUseAuth.mockReturnValue({
      usuario: { id: "u1", nome: "Ana", email: "ana@exemplo.com", tipoUsuario: "candidato", perfilPublico: true, preferenciaMensagens: "todos" },
      sair: mockLogout,
      atualizarUsuario: mockRefreshUser,
    });
    // Sem biometria por padrão; só o `describe` de biometria muda isso.
  ...
  describe("alterar senha", () => {
    ...
    // A troca de senha invalida a sessão no backend (revoga as sessões e recusa os tokens
    // emitidos antes), então a tela não tem o que confirmar: sai da conta com o motivo
    // `senha_alterada`, que é o que a tela de login explica em seguida.
    it("sucesso chama alterarSenha e sai da conta informando a troca de senha", async () => {
      mockCargaPadrao();
      ...
      expect(mockLogout).toHaveBeenCalledWith("senha_alterada");
      expect(getAllByLabelText("Senha atual")[0].props.value).toBe("");
    });
  });

  describe("trocar e-mail", () => {
    it("fluxo completo: solicitar → confirmar → atualizarUsuario", async () => {
      mockCargaPadrao();
  ...
  describe("Segurança do aparelho — bloqueio biométrico", () => {
