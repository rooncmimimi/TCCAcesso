import { Sequelize } from "sequelize";
import env from "./env.js";

/**
 * Instância única do Sequelize.
 *
 * - `underscored: true` liga cada atributo em camelCase à coluna em snake_case de mesmo nome
 *   (`usuarioId` ↔ `usuario_id`, `criadoEm` ↔ `criado_em`), como em `migrations/0001_esquema_inicial.sql`.
 * - Todas as queries são parametrizadas pelo Sequelize (prepared statements),
 *   o que protege contra SQL Injection (OWASP A03).
 */
const sequelize = new Sequelize(
    env.db.name,
    env.db.user,
    env.db.password,
    {
        host: env.db.host,
        port: env.db.port,
        dialect: "postgres",

        dialectOptions: env.db.ssl
            ? {
                  ssl: {
                      require: true,
                      // O Supabase usa certificado gerenciado; em produção
                      // com CA própria troque para `true` e informe o CA.
                      rejectUnauthorized: false
                  }
              }
            : {},

        logging: env.isProducao ? false : (msg) => console.debug(msg),

        define: {
            timestamps: true,
            underscored: true,
            createdAt: "criadoEm",
            updatedAt: "atualizadoEm"
        },

        pool: {
            max: 10,
            min: 0,
            acquire: 30000,
            idle: 10000
        }
    }
);

export default sequelize;
