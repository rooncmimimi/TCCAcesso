...
// O botão "Testar leitura por voz" chama `Speech.speak` direto; mock simples, como em
// `BotaoOuvir.test.tsx`.
...
import { AcessibilidadeScreen } from "../AcessibilidadeScreen";

// `AcessibilidadeProvider` e `TemaProvider` reais: os testes cobrem o caminho completo, da tela ao
// AsyncStorage mockado, e a prévia é conferida de ponta a ponta.
...
describe("AcessibilidadeScreen", () => {
  // O AsyncStorage mockado persiste entre os `it()` do mesmo arquivo. Sem limpar, a preferência
  // `voiceEnabled` ligada num teste vazaria para o próximo, que a desligaria em vez de ligar. O
  // teste de persistência não é afetado, porque só depende das próprias montagens.
  beforeEach(async () => {
    jest.clearAllMocks();
  ...
  it("tema: tocar em 'Escuro' seleciona a opção (accessibilityState) e o preview muda de tema", async () => {
    ...
    expect(estilo.color).toBe("#F3F5F8"); // textPrimary do tema escuro (tema/cores.ts)
  });
  ...
  it("recursos sem aplicação real (cursor ampliado, navegação por teclado) não têm nenhum controle interativo — só texto informativo", async () => {
    ...
    // Nenhum dos dois vira controle; "Fonte para dislexia" e "Leitura por voz" são reais (ver os
    // `describe` abaixo).
    expect(queryByLabelText("Cursor ampliado")).toBeNull();
    expect(queryByLabelText("Navegação por teclado")).toBeNull();
  });

  describe("fonte para dislexia", () => {
