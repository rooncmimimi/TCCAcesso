...
// Fase 17: `PublicProfileScreen` ganhou o botão "Enviar mensagem".
jest.mock("../../mensagens", () => ({
...
// Fase 19: "Bloquear"/"Desbloquear"/"Denunciar".
jest.mock("../../moderacao", () => ({
...
describe("PublicProfileScreen", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockUseAuth.mockReturnValue({ user: { id: "u1", nome: "Ana", email: "ana@exemplo.com", tipoUsuario: "candidato" } });
    mockPodeIniciar.mockResolvedValue({ permitido: true });
    // Fase 19: TODO carregamento de perfil (candidato OU empresa) chama
    // `SeguidorService.resumo` — pra empresa é só por causa de `bloqueado`
    // (`resumoEmpresa` não tem esse campo). Um padrão aqui evita que os
    // testes de empresa fiquem dependendo, por acidente, de sobra de
    // `mockResolvedValue` de um teste de candidato anterior no arquivo
    // (`jest.clearAllMocks()` limpa chamadas, nunca a implementação).
  ...
  describe("bloquear/desbloquear e denunciar (Fase 19)", () => {
