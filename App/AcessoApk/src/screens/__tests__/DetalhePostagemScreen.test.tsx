...
// Mesma técnica de `MensagensScreen.test.tsx`/`FeedScreen.test.tsx`: captura o handler registrado por evento.
jest.mock("../../services/socket/socketClient", () => ({
  ouvirEvento: (...args: unknown[]) => mockOuvirEvento(...args),
}));

// Mesma técnica de `NovaPostagemScreen.test.tsx`: preserva o `AcessibilidadeProvider` real, troca só o anúncio por um espião.
jest.mock("../../acessibilidade", () => ({
  ...jest.requireActual("../../acessibilidade"),
  anunciarParaLeitorDeTela: (...args: unknown[]) => mockAnnounce(...args),
}));

// A tela usa `useAutenticacao()` para esconder "Denunciar" no próprio conteúdo. `"u9"` não é autor
// de nada nestes testes (`u1` e `u2`), então por padrão nenhum conteúdo é do usuário.
jest.mock("../../autenticacao", () => ({
  useAutenticacao: () => ({ usuario: { id: "u9", nome: "Quem Está Vendo", email: "vendo@exemplo.com", tipoUsuario: "candidato" } }),
}));
...
import { DetalhePostagemScreen } from "../DetalhePostagemScreen";

// Confirma a exclusão sem depender de um diálogo nativo, como em `CandidaturasVagaScreen.test.tsx`.
...
describe("DetalhePostagemScreen", () => {
  beforeEach(async () => {
    jest.clearAllMocks();
    alertSpy = jest.spyOn(Alert, "alert");
    mockOuvirEvento.mockReturnValue(() => undefined);
    // Nenhum outro teste depende de `voiceEnabled`; limpar evita vazamento entre eles.
  ...
  describe("editar/excluir a própria publicação", () => {
  ...
  describe("excluir o próprio comentário", () => {
    ...
    it("mostra 'Excluir' só no próprio comentário, nunca no dos outros", async () => {
      ...
      // `getByRole` lança se houver mais de um, então passar já prova que só o meu comentário tem
      // "Excluir".
      ...
      mockListarComentarios.mockResolvedValue(
        envelopeComentarios([
          comentario({
            ...
            respostas: [{ id: "rmeu", comentario: "Minha resposta.", criadoEm: "2026-01-01T02:00:00.000Z", usuario: { id: "u9", nome: "Quem Está Vendo" } }],
          }),
        ]),
      );
      mockRemoverComentario.mockResolvedValue({ mensagem: "Comentário removido com sucesso." });
  ...
  describe("anexos", () => {
    ...
    it("um anexo de vídeo (criado por outro cliente) mostra um aviso em texto, sem tentar exibir imagem nenhuma", async () => {
      mockObterPorId.mockResolvedValue(
        postagem({ anexos: [{ id: "a2", postagemId: "p1", tipo: "video", url: "https://x/v.mp4", descricao: null, ordem: 0 }] }),
      );
      mockListarComentarios.mockResolvedValue(envelopeComentarios([]));
      const { findByText } = await renderTela();
