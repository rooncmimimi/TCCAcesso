/**
 * Erro de aplicação com status HTTP.
 *
 * Sempre lance ErroApi nos Services. O erroMiddleware usa
 * `statusCode` para responder corretamente e trata qualquer
 * outro erro como 500 (sem vazar detalhes internos).
 */
class ErroApi extends Error {
    constructor(statusCode, message, detalhes = null) {
        super(message);

        this.name = "ApiError";
        this.statusCode = statusCode;
        this.detalhes = detalhes;
        this.isOperational = true;

        Error.captureStackTrace(this, this.constructor);
    }

    static requisicaoInvalida(mensagem, detalhes) {
        return new ErroApi(400, mensagem, detalhes);
    }

    static naoAutenticado(mensagem = "Não autenticado.", detalhes) {
        return new ErroApi(401, mensagem, detalhes);
    }

    static acessoNegado(mensagem = "Acesso negado.", detalhes) {
        return new ErroApi(403, mensagem, detalhes);
    }

    static naoEncontrado(mensagem = "Recurso não encontrado.") {
        return new ErroApi(404, mensagem);
    }

    static conflito(mensagem) {
        return new ErroApi(409, mensagem);
    }

    static servicoIndisponivel(mensagem = "Serviço temporariamente indisponível.") {
        return new ErroApi(503, mensagem);
    }

    /**
     * Erro interno (500) com mensagem amigável para o cliente, preservando
     * o erro original (`causaOriginal`) apenas para log; nunca é enviado
     * na resposta HTTP. Use quando uma falha inesperada (ex.: erro de
     * banco de dados) precisa virar uma mensagem específica para o
     * usuário, sem esconder a causa raiz dos logs do servidor.
     */
    static interno(mensagem = "Erro interno do servidor.", causaOriginal = null) {
        const erro = new ErroApi(500, mensagem);
        erro.causaOriginal = causaOriginal;
        return erro;
    }
}

export default ErroApi;
