...
// Guarda o handler registrado para cada evento, para cada teste disparar o evento manualmente
// (mesma técnica de `MensagensScreen.test.tsx`).
...
// Cast proposital, como em `VagasScreen.test.tsx`: a tela só usa o `navigate` de `navigation`.
const navigationMock = {
  navigate: mockNavigate,
} as unknown as NativeStackScreenProps<AppStackParamList, "Tabs">["navigation"] &
  NativeStackScreenProps<AbasParamList, "Home">["navigation"];
  ...
  const utils = await render(
    <AcessibilidadeProvider>
      <TemaProvider>
        ...
        <FeedScreen navigation={navigationMock} route={{ key: "Home", name: "Home" }} />
      </TemaProvider>
    </AcessibilidadeProvider>,
  );
...
describe("FeedScreen", () => {
  ...
  // `fireEvent(lista, "refresh")` não aciona o `RefreshControl` nativo, então chama o `onRefresh`
  // direto pela prop, como nos testes de `MensagensScreen` e `NotificacoesScreen`.
  ...
  it("mostra o tempo relativo da publicação abaixo do nome do autor", async () => {
    mockListar.mockResolvedValue(envelope([postagem({ criadoEm: new Date().toISOString() })]));
    const { findByText } = await renderTela();
  ...
  it("toque numa publicação navega para PostagemDetail com o postagemId certo", async () => {
    ...
    // Regex, e não texto exato: o rótulo inclui o tempo relativo da publicação, que depende da data
    // e do fuso do ambiente; só o prefixo com o autor é estável.
  ...
  describe("anexos", () => {
    it("mostra a primeira imagem do anexo, com a descrição real como accessibilityLabel", async () => {
      mockListar.mockResolvedValue(
        envelope([
          postagem({
            anexos: [{ id: "a1", postagemId: "p1", tipo: "imagem", url: "https://x/a1.jpg", descricao: "Um pôr do sol.", ordem: 0 }],
          }),
        ]),
      );
    ...
    it("anexo sem descrição usa um aviso honesto como accessibilityLabel (nunca fica muda)", async () => {
      mockListar.mockResolvedValue(
        envelope([
          postagem({
            anexos: [{ id: "a1", postagemId: "p1", tipo: "imagem", url: "https://x/a1.jpg", descricao: null, ordem: 0 }],
          }),
        ]),
      );
      const { findByLabelText } = await renderTela();
    ...
    it("mais de um anexo mostra '+N anexos' abaixo da primeira imagem", async () => {
      mockListar.mockResolvedValue(
        envelope([
          postagem({
            anexos: [
              { id: "a1", postagemId: "p1", tipo: "imagem", url: "https://x/a1.jpg", descricao: "Foto 1", ordem: 0 },
              { id: "a2", postagemId: "p1", tipo: "imagem", url: "https://x/a2.jpg", descricao: "Foto 2", ordem: 1 },
            ],
          }),
        ]),
      );
    ...
    it("postagem sem nenhum anexo não renderiza nenhuma imagem extra", async () => {
      mockListar.mockResolvedValue(envelope([postagem()]));
