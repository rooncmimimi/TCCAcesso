...
import { DetalheVagaScreen } from "../DetalheVagaScreen";
...
describe("DetalheVagaScreen", () => {
  beforeEach(async () => {
    jest.clearAllMocks();
    mockUseAuth.mockReturnValue({ usuario: candidato });
    // Nenhum teste depende de `voiceEnabled`; limpar evita vazamento entre eles, como em
    // `AcessibilidadeScreen.test.tsx`.
  ...
  it("sem data de criação, não mostra nenhuma data", async () => {
    mockObterPorId.mockResolvedValue(vaga({ criadoEm: null }));
    const { findByText, queryByText } = await renderTela();
  ...
  it("falha ao favoritar não muda o estado visual — só mostra o erro", async () => {
    mockObterPorId.mockResolvedValue(vaga());
    // Sem `.response`: `extrairMensagemErro` cai na mensagem de "sem conexão" (comportamento real, documentado em `services/api/erros.ts`).
    const erro = Object.assign(new Error("falhou"), { isAxiosError: true });
  ...
  it("usuário empresa não vê os controles de candidatura/favoritar", async () => {
    mockUseAuth.mockReturnValue({ usuario: empresaUsuario });
    mockObterPorId.mockResolvedValue(vaga());
    const { findByText, queryByRole } = await renderTela();
