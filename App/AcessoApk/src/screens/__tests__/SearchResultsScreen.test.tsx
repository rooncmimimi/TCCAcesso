...
async function renderTela(tipo: TipoBusca, termo = "dev") {
  const utils = await render(
    <AccessibilityProvider>
      <ThemeProvider>
        <SearchResultsScreen navigation={navigationMock} route={{ key: "SearchResults", name: "SearchResults", params: { termo, tipo } }} />
      </ThemeProvider>
    </AccessibilityProvider>,
  );
...
describe("SearchResultsScreen", () => {
  ...
  it("com mais de uma página, 'Próxima página' busca a página 2", async () => {
    mockBuscarPostagens.mockResolvedValueOnce(
      envelope("postagens", [{ id: "p1", conteudo: "Olá mundo", created_at: "2026-01-01T00:00:00.000Z", usuario: { nome: "Ana" } }], {
        totalPaginas: 2,
        total: 11,
      }),
    );
    ...
    mockBuscarPostagens.mockResolvedValueOnce(
      envelope("postagens", [{ id: "p2", conteudo: "Segunda página", created_at: "2026-01-02T00:00:00.000Z", usuario: { nome: "Bia" } }], {
        pagina: 2,
        totalPaginas: 2,
        total: 11,
      }),
