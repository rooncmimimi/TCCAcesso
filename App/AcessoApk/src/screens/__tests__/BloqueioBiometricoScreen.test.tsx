...
jest.mock("../../autenticacao", () => ({
  useAutenticacao: () => ({ sair: mockLogout }),
}));

jest.mock("../../acessibilidade", () => ({
  ...jest.requireActual("../../acessibilidade"),
  anunciarParaLeitorDeTela: (...a: unknown[]) => mockAnnounce(...a),
}));
...
import { BloqueioBiometricoScreen } from "../BloqueioBiometricoScreen";

async function renderTela() {
  const utils = await render(
    <AcessibilidadeProvider>
      <TemaProvider>
        <BloqueioBiometricoScreen />
      </TemaProvider>
    </AcessibilidadeProvider>,
  );
...
describe("BloqueioBiometricoScreen", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
